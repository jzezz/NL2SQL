...
def _random_date_within_last_year():
    days_ago = random.randint(0, 365)
    return datetime.now() - timedelta(days=days_ago)


def _maybe_null(value, probability=0.2):
    return value if random.random() > probability else None


def _insert_doctors(cursor):
    doctors = []
    for _ in range(15):
...
def _insert_patients(cursor):
    patients = []
    for _ in range(200):
        first = random.choice(FIRST_NAMES)
        last = random.choice(LAST_NAMES)
        email = _maybe_null(f"{first.lower()}.{last.lower()}@mail.com")
        phone = _maybe_null(f"9{random.randint(100000000, 999999999)}")
        dob = datetime.now() - timedelta(days=random.randint(18*365, 70*365))
        gender = random.choice(["M", "F"])
        city = random.choice(CITIES)
        registered = _random_date_within_last_year()
        patients.append((
...
def _insert_appointments(cursor):
    appointments = []
    for _ in range(500):
        patient_id = random.randint(1, 200)
        doctor_id = random.randint(1, 15)
        date = _random_date_within_last_year()
        status = random.choices(STATUSES, weights=[0.3, 0.5, 0.1, 0.1])[0]
        notes = _maybe_null("Follow-up required", 0.5)
...
def _insert_treatments(cursor):
    ...
    for _ in range(350):
        ...
        treatments.append((appointment_id, name, cost, duration))
...
def _insert_invoices(cursor):
    invoices = []
    for _ in range(300):
        patient_id = random.randint(1, 200)
        invoice_date = _random_date_within_last_year()
...
def initialize_database():
    ...
    doc_count = _insert_doctors(cursor)
    pat_count = _insert_patients(cursor)
    app_count = _insert_appointments(cursor)
    trt_count = _insert_treatments(cursor)
    inv_count = _insert_invoices(cursor)

    conn.commit()
    conn.close()

    print(
        f"Initialized database: {pat_count} patients, "
        f"{doc_count} doctors, {app_count} appointments, "
        f"{trt_count} treatments, {inv_count} invoices."
    )